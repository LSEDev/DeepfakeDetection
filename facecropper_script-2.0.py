...
def extract_face(data_path, output_path, imgname):
    ...
    for (x, y, w, h) in faces:
        ...
        #faceimg = img[int(0.5*ny):int(1.5*(ny+nr)), 
        #              int(0.5*nx):int(1.5*(nx+nr))]
        shortest_side = min(int(1.2*(ny+nr)) - int(0.8*ny),
                           int(1.2*(nx+nr)) - int(0.8*nx))
        
        lastimg = img[int(centery-shortest_side/2):int(centery+shortest_side/2),
                     int(centerx-shortest_side/2):int(centerx+shortest_side/2)]
