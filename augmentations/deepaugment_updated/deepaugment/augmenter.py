...
def transform(aug_type, magnitude, X):
    ...
    if aug_type == "crop":
        ...
        X_aug = iaa.Affine(translate_percent={"x": (-magnitude, magnitude), "y": (0, 0)}).augment_images(X)
        
    elif aug_type == "translate-y":
        X_aug = iaa.Affine(translate_percent={"x": (0, 0), "y": (-magnitude, magnitude)}).augment_images(X)
        ...
        X_aug = iaa.Dropout((0.01, max(0.011, magnitude)), per_channel=0.5).augment_images(X)
        #first argument should be smaller than second one
    ...
    #####################################################################
        ...
        # new code:
        X_aug = iaa.Invert(1.0).augment_images(X.astype(np.uint8))
    #####################################################################
    ...
    #####################################################################
        ...
        # new code:
        X_aug = iaa.Superpixels(p_replace=(0, magnitude), n_segments=(100, 100)).augment_images(X.astype(np.uint8))
    #####################################################################
    ...
    #####################################################################
        ...
        # new code:
        X_aug = iaa.AddToHueAndSaturation((int(-45 * magnitude), int(45 * magnitude))).augment_images(X.astype(np.uint8))
    #####################################################################
    ...
    #####################################################################
        ...
        # new code:
        X_aug = iaa.Grayscale(alpha=(0.0, magnitude)).augment_images(X.astype(np.uint8))
    #####################################################################
    
    elif aug_type == "fog":
        X_aug = iaa.Fog().augment_images(X)

    elif aug_type == "clouds":
        X_aug = iaa.Clouds().augment_images(X)
